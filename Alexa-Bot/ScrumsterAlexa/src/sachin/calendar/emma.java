/**
 * emma.java handles meeting bookings via integrations with Amazon Alexa and Google Calendar API.
...
public class emma {
    ...
    */
    private static List<Integer> slot(com.google.api.services.calendar.Calendar service, DateTime min_time, DateTime max_time, String zone_val,
                                      List<String> items, List<Long> epoch_val, boolean user, long epoch_now) throws Exception {
    ...
    */
    private static List<Integer> event_check(com.google.api.services.calendar.Calendar service, DateTime min_time, DateTime max_time,
        ...
        List<Integer> busy_slots = new ArrayList<>();
    ...
    private static void create_event(com.google.api.services.calendar.Calendar service, List<String> users, String zone, DateTime startDateTime,
                                     DateTime endDateTime, String name, String room) throws IOException, ParseException {
    ...
    public static void main(String[] args) throws Exception {
        ...
        List<Integer> user_slot = slot(service, min_time, max_time, zone, users, epoch_values, true, now);
        List<Integer> room_slot1 = slot(service, min_time, max_time, zone, room1, epoch_values, false, now);


        //Create HashSet and list to obtain available slots for booking
        Set<Integer> user_slot_hash1 = new HashSet<>();
        Set<Integer> room_slot_hash1 = new HashSet<>();
        List<Integer> book_slot1 = new ArrayList<>();

        user_slot_hash1.addAll(user_slot);
        room_slot_hash1.addAll(room_slot1);
        user_slot_hash1.retainAll(room_slot_hash1);

        book_slot1.addAll(user_slot_hash1);
        System.out.println("book-slot1 = " + book_slot1);
        Collections.sort(book_slot1);
        if (!book_slot1.isEmpty()) {
            System.out.println(book_slot1.get(0));
            DateTime start_date1 = GetFormat(date, book_slot1.get(0), true);
            System.out.println("Format output " + start_date1);
            DateTime end_date1 = GetFormat(date, book_slot1.get(0), false);
            System.out.println("Format output " + end_date1);
            create_event(service, users, zone, start_date1, end_date1, "primary", room1.get(0));
            return;
        }

        List<Integer> room_slot2 = slot(service, min_time, max_time, zone, room2, epoch_values, false, now);

        //Create HashSet and list to obtain available slots for booking
        Set<Integer> user_slot_hash2 = new HashSet<>();
        Set<Integer> room_slot_hash2 = new HashSet<>();
        List<Integer> book_slot2 = new ArrayList<>();

        user_slot_hash2.addAll(user_slot);
        room_slot_hash2.addAll(room_slot2);
        user_slot_hash2.retainAll(room_slot_hash2);

        book_slot2.addAll(user_slot_hash2);
        System.out.println("book-slot2 = " + book_slot2);
        Collections.sort(book_slot2);
        if (!book_slot2.isEmpty()) {
            System.out.println(book_slot2.get(0));
            DateTime start_date2 = GetFormat(date, book_slot2.get(0), true);
            DateTime end_date2 = GetFormat(date, book_slot2.get(0), false);
            create_event(service, users, zone, start_date2, end_date2, "primary", room2.get(0));
        }
